...
export class CLICommandExecutor {
  ...
  protected async buildContext(
    ...
    const params = paramsCtor ? new paramsCtor(positional, flags) : new (class extends CommandParams<unknown[], Record<string, unknown>> {
      constructor() {
  ...
  protected async runLifecycle(
    ...
    const result = typeof cmd.DryRun === "function" && ctx.Params.DryRun ? await cmd.DryRun(ctx, this.ioc) : await cmd.Run(ctx, this.ioc);
