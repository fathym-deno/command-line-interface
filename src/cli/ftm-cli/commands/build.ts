...
export default Command("build", "Prepare static CLI build folder")
  ...
  .Services(async (ctx, ioc) => {
    ...
    const buildDFS: DFSFileHandler = ctx.Params.ConfigOverride ? await dfsCtx.GetDFS("CLI") : await dfsCtx.GetExecutionDFS();

    const { configPath, outDir, configDir, templatesDir } = await resolveConfigAndOutDir(ctx.Params, buildDFS);
