...
Deno.test("LocalDevCLIFileSystemHooks", async (t) => {
  ...
  await t.step(
    "ResolveCommandEntryPaths – maps commands and metadata",
    async () => {
  ...
  await t.step(
    "LoadCommandModule – loads .metadata.ts group module",
    async () => {
      const { hooks } = createHooks();
  ...
  await t.step("ResolveCommandEntryPaths – applies Root prefix", async () => {
    ...
    assert(
      extGroup?.GroupPath,
      "ext should have GroupPath from root .metadata.ts",
    );
    assertEquals(
      extGroup?.ParentGroup,
      undefined,
      "ext should have no parent (top-level)",
    );
    ...
    assertEquals(
      ...
      "ext",
      'ext/plugin parent should be "ext"',
    );

    // plugin/deploy.ts should become "ext/plugin/deploy"
    assert(
      entries.has("ext/plugin/deploy"),
      'Should have "ext/plugin/deploy" key',
    );
    ...
    assertEquals(
      deploy?.ParentGroup,
      "ext",
      'ext/plugin/deploy parent should be "ext"',
    );
  });

  await t.step(
    "ResolveCommandEntryPaths – handles nested Root prefix",
    async () => {
      const { hooks } = createHooks();
      ...
      assertEquals(
        ...
        "plugins",
        'plugins/v2 parent should be "plugins"',
      );

      // plugin/deploy.ts should become "plugins/v2/plugin/deploy"
      assert(
        entries.has("plugins/v2/plugin/deploy"),
        "Should have nested command key",
      );
      const deploy = entries.get("plugins/v2/plugin/deploy");
      assertEquals(
        deploy?.ParentGroup,
        "plugins",
        'Nested command parent should be "plugins"',
      );
    },
  );
