...
describe('SchemaIntrospector', () => {
  ...
  describe('isComplexType', () => {
    it('returns true for ZodObject', () => {
      assertEquals(introspector.isComplexType(z.object({ key: z.string() })), true);
    });
    ...
    it('returns true for ZodRecord', () => {
      assertEquals(introspector.isComplexType(z.record(z.string(), z.string())), true);
    });
  ...
  describe('shouldFileCheck', () => {
    ...
    it('respects explicit fileCheck: true on primitives', () => {
      assertEquals(introspector.shouldFileCheck(z.string().meta({ fileCheck: true })), true);
    });

    it('respects explicit fileCheck: false on complex types', () => {
      assertEquals(introspector.shouldFileCheck(z.object({}).meta({ fileCheck: false })), false);
    });
  });
