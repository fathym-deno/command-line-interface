...
```typescript
import { TelemetryLogAdapter, createCliTelemetryLogger } from '@fathym/cli';
...
```typescript
import { ArcSpinner, WindowsSpinner, DotsSpinner } from '@fathym/cli';
...
// Too verbose
Log.Info('The build process is now starting and will compile all TypeScript modules...');
```
